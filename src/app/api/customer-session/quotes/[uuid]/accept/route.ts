...
export async function PATCH(req: Request, context: { params: Promise<{ uuid: string }> }) {
  ...
  try {
    const response = await fetch(`${apiBase()}/quotes/${uuid}/accept`, { method: "PATCH", headers: { Authorization: `Bearer ${token}` }, cache: "no-store", signal: AbortSignal.timeout(15000) });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) return sessionJson({ message: backendErrorMessage(body, "Could not accept this quote.") }, response.status);
